...
The challenge, however, is how to make sense of the Inverse Transform Theorem. On first learning, I found this theorem deceptively and I struggled to grasp the key ideas behind it. Here is the theorem explained:

> Let $X$ be a continuous random variable with c.d.f $F(x)$. Then $F(X) \sim Unif(0,1)$.

And, here's the equally perplexing proof:

> Let $Y = F(X)$ and suppose that $Y$ has a c.d.f $G(y)$, then
> $$G(y) = P(Y \leq y) = P(F(Y) \leq y) $$
> $$ = P(X \leq F^{-1}(y)) = F(F^{-1}(y)) = y$$

Let me explain the mechanics of the theorem. This theorem simply states that given we have a cumulative density function of a continuous random variable, the inverse of the CDF function will produce a Uniform disteribution. While the mechanics is rather straightforward, I had so many questions about why this works:
...
And what I see as a recurring theme amongst SoME submissions, is that for many mathematical concepts that appear hard to understand in written form, we can achieve some clarity expressing the ideas in visual form. Certainly, this is what I experienced as I sought to better understand this theorem.
...
- The $Unif(0,1)$ distribution produces a random number between 0 and 1
- Imagine that number on the Y axis of a CDF for your distribution of interest
- The inverse function $F^{-1}(x)$ transforms this number to a value on the X axis
- Which in turn becomes a random variate from your distribution of interest
