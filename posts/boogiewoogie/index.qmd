...
You probably realize that my simple artwork is very similar to Piet Mondrian's iconic [Broadway Boogie Woogie](https://en.wikipedia.org/wiki/Broadway_Boogie_Woogie). Broadway Boogie Woogie is considered one of Mondrian's most emblematic paintings and its aesthetic is instantly recognised all around the world. The original piece is housed today in New York's [Museum of Modern Art (MOMA)](https://www.moma.org/collection/works/78682).
...
And this is the most fascinating part of the equation for me. The original art piece alone is beautiful but in the process of recreating the same art piece using tools that are unique to Tom Sachs, he created his Genuine Fake. It's a `Fake` in the sense that it is not the original, but it is also `Genuine` because Tom Sachs experienced the joy of creating something uniquely his through intense study of the original.
...
In particular, I am taking a course on [Data and Visual Analytics](https://omscs.gatech.edu/cse-6242-data-visual-analytics) this semester, which has a strong focus on [D3.js](https://d3js.org/), a javascript visualisation framework, so that's what I used to build **my genuine fake**.

# Interesting things I learnt along the way

Ok, that's enough rambling about **Why** I started this project, let me now shift gears to list some of my learnings as I worked on **How** to execute this project.
...
Above, you can also see some of the niceties ObservableJS brings, such as the [Hypertext Literals](https://observablehq.com/@observablehq/htl) which makes it a breeze to create HTML elements directly in the `{ojs}` code block without the need to touch the raw HTML file.
...
In my implementation, I relied on Javascript's `Math.random()` random number generator, which I later learnt is pseudo uniform. D3 includes a [d3-random](https://github.com/d3/d3-random) which exposes more functions from a greater variety of distributions, and I would experiment with these if I were to revisit this project in the future.
